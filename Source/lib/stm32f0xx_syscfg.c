...
{
  uint32_t tmp = 0x00;
  uint8_t cr_num = EXTI_PinSourcex >> 0x02;
  uint32_t quik_maf = (((uint32_t)EXTI_PortSourceGPIOx) << (0x04 * (EXTI_PinSourcex & (uint8_t)0x03)));
...
/**
  ...
  */
  /* This one register does not seem to be included in the STM32F03 chips, so I commented it out
